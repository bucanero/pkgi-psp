...
static void pkgi_do_main(pkgi_input* input)
{
    ...
    if (input)
    {
        if (input->active & pkgi_cancel_button())
        {
            ...
            pkgi_dialog_ok_cancel("\xE2\x98\x85  PKGi PSP v" PKGI_VERSION "  \xE2\x98\x85", _("Exit to XMB?"), &cb_dialog_exit);
        }
    ...
    for (uint32_t i = first_item; i < db_count; i++)
    {
        ...
        if (i == selected_item)
        {
            pkgi_draw_fill_rect_z(0, y, PKGI_FONT_Z, PKGI_SCREEN_WIDTH, font_height + PKGI_MAIN_ROW_PADDING - 1, PKGI_COLOR_SELECTED_BACKGROUND);
        }
        ...
        pkgi_clip_set(0, y, PKGI_SCREEN_WIDTH, line_height);
        ...
        pkgi_draw_text(PKGI_SCREEN_WIDTH - (PKGI_MAIN_SCROLL_WIDTH + PKGI_MAIN_SCROLL_PADDING + PKGI_MAIN_HMARGIN + sizew), y, color, size_str);
        pkgi_clip_remove();

        pkgi_clip_set(col_name, y, PKGI_SCREEN_WIDTH - PKGI_MAIN_SCROLL_WIDTH - PKGI_MAIN_SCROLL_PADDING - PKGI_MAIN_COLUMN_PADDING - sizew - col_name, line_height);
        pkgi_draw_text_ttf(0, 0, PKGI_FONT_Z, color, item->name);
        pkgi_clip_remove();

        y += font_height + PKGI_MAIN_ROW_PADDING;
        if (y > PKGI_SCREEN_HEIGHT - (font_height + PKGI_MAIN_HLINE_EXTRA*6 + PKGI_MAIN_VMARGIN))
        {
            break;
        }
        else if (y + font_height > PKGI_SCREEN_HEIGHT - (font_height + PKGI_MAIN_HLINE_EXTRA))
        {
            line_height = (PKGI_SCREEN_HEIGHT - (font_height + PKGI_MAIN_HLINE_EXTRA)) - (y + 1);
    ...
    if (db_count == 0)
    {
        ...
        pkgi_draw_text((PKGI_SCREEN_WIDTH - w) / 2, PKGI_SCREEN_HEIGHT / 2, PKGI_COLOR_TEXT, text);
    }
    ...
    if (db_count != 0)
    {
        ...
        if (max_items < db_count)
        {
            ...
            pkgi_draw_fill_rect_z(PKGI_SCREEN_WIDTH - (PKGI_MAIN_HMARGIN + PKGI_MAIN_SCROLL_WIDTH), font_height + PKGI_MAIN_HLINE_EXTRA + PKGI_MAIN_VMARGIN + start + 2, PKGI_FONT_Z, PKGI_MAIN_SCROLL_WIDTH, height, PKGI_COLOR_SCROLL_BAR);
        }
    }
...
{
    pkgi_draw_text((PKGI_SCREEN_WIDTH - pkgi_text_width(error_state)) / 2, PKGI_SCREEN_HEIGHT / 2, PKGI_COLOR_TEXT_ERROR, error_state);
}
...
int main(int argc, char* argv[])
{
    ...
    avail_height = PKGI_SCREEN_HEIGHT - 2 * (font_height + PKGI_MAIN_HLINE_EXTRA*2 + PKGI_MAIN_VMARGIN);
    bottom_y = PKGI_SCREEN_HEIGHT - (PKGI_MAIN_VMARGIN + font_height);
