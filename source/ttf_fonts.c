...
int TTFLoadFont(int set, const char * path, void * from_memory, int size_from_memory)
{
    ...
    if(path) {
        if((file_exist(path) == 0) || FT_New_Face(freetype, path, 0, &face[set])<0) return -1;
    } else {
...
bitmap: uint8_t bitmap passed to render the character character (max 256 x 256 x 1 (8 bits Alpha))
    ...
    while(*string) {
        ...
        if(*string & 128) {
            ...
            if((*string & 0xf8)==0xf0) { // 4 bytes
                ttf_char = (uint32_t) (*(string++) & 3);
                m = 3;
            } else if((*string & 0xE0)==0xE0) { // 3 bytes
                ttf_char = (uint32_t) (*(string++) & 0xf);
                m = 2;
            } else if((*string & 0xE0)==0xC0) { // 2 bytes
                ttf_char = (uint32_t) (*(string++) & 0x1f);
                m = 1;
             ...
             for(n = 0; n < m; n++) {
                if(!*string) break; // error!
                    ...
                    ttf_char = (ttf_char <<6) |((uint32_t) (*(string++) & 63));
             }
           
            if((n != m) && !*string) break;
        
        } else ttf_char = (uint32_t) *(string++);
        ...
        if(ttf_char!=0 && slot->bitmap.buffer) {
            ...
            for(n = 0; n < slot->bitmap.rows; n++) {
                ...
                for (m = 0; m < slot->bitmap.width; m++) {
                    ...
                    color = (uint8_t) slot->bitmap.buffer[ww + m];
...
static int Win_W_ttf = PKGI_SCREEN_WIDTH;
static int Win_H_ttf = PKGI_SCREEN_HEIGHT;


static uint32_t Win_flag = 0;
...
static void DrawBox_ttf(float x, float y, float z, float w, float h, uint32_t rgba)
{
    SDL_FRect rect = {
        .x = x,
        .y = y,
...
static SDL_Texture* create_texture(const uint8_t* bitmap, uint32_t rgba)
{
    uint32_t buf[TEX_SZ * TEX_SZ];
...
static void DrawTextBox_ttf(SDL_Texture* bitmap, float x, float y, float z, float w, float h, uint32_t rgba, float tx, float ty)
{
    SDL_FRect dest = {
        .x = x,
        .y = y,
...
int display_ttf_string(int posx, int posy, const char *string, uint32_t color, uint32_t bkcolor, int sw, int sh, int (*DrawIcon_cb)(int, int, char))
{
    int l,n, m, ww, ww2;
    uint8_t colorc;
    uint32_t ttf_char;
    uint8_t *ustring = (uint8_t *) string;
    ...
    while(*ustring) {
        ...
        if(*ustring & 128) {
            ...
            if((*ustring & 0xf8)==0xf0) { // 4 bytes
                ttf_char = (uint32_t) (*(ustring++) & 3);
                m = 3;
            } else if((*ustring & 0xE0)==0xE0) { // 3 bytes
                ttf_char = (uint32_t) (*(ustring++) & 0xf);
                m = 2;
            } else if((*ustring & 0xE0)==0xC0) { // 2 bytes
                ttf_char = (uint32_t) (*(ustring++) & 0x1f);
                m = 1;
             ...
             for(n = 0; n < m; n++) {
                if(!*ustring) break; // error!
                    ...
                    ttf_char = (ttf_char <<6) |((uint32_t) (*(ustring++) & 63));
             }
           
            if((n != m) && !*ustring) break;
        
        } else ttf_char = (uint32_t) *(ustring++);
            ...
            if(ttf_char!=0) {
                ...
                for(n = 0; n < slot->bitmap.rows; n++) {
                    ...
                    for (m = 0; m < slot->bitmap.width; m++) {
                        ...
                        colorc = (uint8_t) slot->bitmap.buffer[ww + m];
        ...
        uint32_t ccolor = color;
        uint32_t cx =(ttf_font_datas[l].width * sw / TEX_SZ) + 1;
