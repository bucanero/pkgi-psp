...
struct t_font_description
{
    ...
    uint8_t* rsx_text_offset;
    uint32_t rsx_bytes_per_char; 
...
void DrawTexture(pkgi_texture tex, int x, int y, int z, int w, int h, uint32_t rgba)
{
	SDL_Rect dest = {
		.x = x,
		.y = y,
...
uint8_t * AddFontFromBitmapArray(uint8_t *font, uint8_t *texture, uint8_t first_char, uint8_t last_char, int w, int h, int bits_per_pixel, int byte_order)
{
    int n, a, b;
    uint32_t buf[w*h];
    uint8_t i;
    ...
    for(n = first_char; n <= last_char; n++) {
        ...
        texture = (uint8_t *) ((((long) texture) + 15) & ~15);
...
/*
uint8_t * AddFontFromTTF(uint8_t *texture, uint8_t first_char, uint8_t last_char, int w, int h, 
    void (* ttf_callback) (uint8_t chr, uint8_t * bitmap, short *w, short *h, short *y_correction))
{
    int n, a, b;
    uint8_t i;
    uint8_t *font;
    static uint8_t letter_bitmap[257 * 256];
    ...
    for(n = first_char; n <= last_char; n++) {
		...
		ttf_callback((uint8_t) (n & 255), letter_bitmap, &font_datas.fonts[font_datas.number_of_fonts].fw[n], &hh,  &font_datas.fonts[font_datas.number_of_fonts].fy[n]);
...
void SetFontColor(uint32_t color, uint32_t bkcolor)
{
...
void RegisterSpecialCharacter(char value, short fy, float scale, pkgi_texture image)
{
...
int WidthFromStrMono(uint8_t * str)
{
    int w = 0;
...
static void orbis2dDrawChar(float x, float y, const uint8_t* bitmap, int bw, int bh, int dw, int dh, uint32_t rgba)
{
...
void DrawCharMono(float x, float y, float z, uint8_t chr)
{
...
void DrawChar(float x, float y, float z, uint8_t chr)
{
...
static int i_must_break_line(const char *str, float x)
{
    ...
    while(*str) {
        if(((uint8_t)*str) <= 32) break;
        xx += dx * font_datas.fonts[font_datas.current_font].fw[((uint8_t)*str)] / font_datas.fonts[font_datas.current_font].w;
        str++;
    }
...
float DrawStringMono(float x, float y, const char *str)
{
    ...
    case FONT_ALIGN_SCREEN_CENTER:
        x= (PKGI_SCREEN_WIDTH - WidthFromStrMono((uint8_t *) str)) / 2;
        break;

    case FONT_ALIGN_RIGHT:
		x -= WidthFromStrMono((uint8_t *) str);
        break;

    case FONT_ALIGN_CENTER:
		x -= WidthFromStrMono((uint8_t *) str)/2;
        break;
    ...
    while (*str) {
        ...
        DrawChar(x, y, font_datas.Z, (uint8_t) *str);
...
float DrawString(float x, float y, const char *str)
{
    ...
    {
    case FONT_ALIGN_SCREEN_CENTER:
        x= (PKGI_SCREEN_WIDTH - WidthFromStr(str)) / 2;
        break;
...
float DrawFormatStringMono(float x, float y, char *format, ...)
{
    ...
    while (*str) {
        DrawCharMono(x, y, font_datas.Z, (uint8_t) *str);
        x += font_datas.sx;
        str++;
    }
